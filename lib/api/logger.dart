...
class AppLogger {
  ...
  static void request(String method, String path, {dynamic data, Map<String, dynamic>? queryParams}) {
    if (!_enabled) return;
  ...
  static void response(String method, String path, int? statusCode, dynamic data) {
    if (!_enabled) return;
    _printBlock('✅ RESPONSE', '$method $path [$statusCode]', {'data': _truncate(data)});
  }

  static void error(String method, String path, Object error, {StackTrace? stackTrace}) {
    if (!_enabled) return;
    _printBlock(
      '❌ ERROR',
      '$method $path -> $error',
  ...
  static void _printBlock(String tag, String message, Map<String, dynamic>? extra) {
