...
import 'package:obecno/features/more/data/models/privacy_model.dart';
import 'package:flutter_secure_storage/flutter_secure_storage.dart';

class PrivacyService {
  PrivacyService(this._client, {FlutterSecureStorage? storage})
    : _storage = storage ?? const FlutterSecureStorage();

  final ApiClient _client;
  final FlutterSecureStorage _storage;

  static const _keyContent = 'privacy_content_v1';
  static const _keyUpdatedAt = 'privacy_updated_at_v1';
  ...
  Future<ApiResponse<PrivacyModel>> fetchFromApi() async {
    try {
      ...
      if (!success || body is! Map<String, dynamic>) {
        return ApiResponse.failure(
          (decoded['message'] as String?) ?? 'Failed to load Privacy Policy.',
          statusCode: response.statusCode,
        );
      }

      return ApiResponse.success(
        PrivacyModel.fromApiJson(body),
        statusCode: response.statusCode,
      );
    } on ApiError catch (e) {
  ...
  Future<PrivacyModel?> getCached() async {
    final content = await _storage.read(key: _keyContent);
    if (content == null || content.isEmpty) return null;

    final updatedAt = await _storage.read(key: _keyUpdatedAt);
    return PrivacyModel.fromCache(content: content, updatedAt: updatedAt);
  }

  Future<void> cache(PrivacyModel model) async {
