...
class DeviceProvider extends BaseProvider {
      ...
      _devices = _devices
          .map((d) {
            final isMatch =
                ...
                (fallback.deviceId.isNotEmpty &&
                    d.deviceId == fallback.deviceId) ||
            ...
            if (updated.requestedAt == null && fallback.requestedAt != null) {
              updated = updated.withTimestamps(
  ...
  Future<bool> checkDeviceStatus(
    ...
    // Later polls (10s BACKGROUND, lifecycle resume, linked-devices
    // refresh) exist to catch a manager *block*, not to re-toast
    // "you logged in from unregistered device" while the user is already in
    // the app. Show that toast only on a real open: login or cold start.
    const unregisteredAlertSources = {'APP_START', 'LOGIN'};
    if (!unregisteredAlertSources.contains(source)) {
      AppLogger.info(
        '[DeviceProvider] checkDeviceStatus: '
        '${pending ? "pending" : "unregistered"} ($source, no alert)',
      );
      return true;
    }
    ...
    AppLogger.info(
      ...
      '(source=$source, loginMessage=$loginMessage)',
    );
    DeviceApprovalGuard.handle(
      context: null,
